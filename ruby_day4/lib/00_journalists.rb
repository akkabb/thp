...
def total_handles
    puts "Dans cette array  il y a : #{handles.count}"
end

def min_handles
    puts "Le handle le plus court de la liste est : #{handles.min {|a, b| a.size <=> b.size}}" #{|a, b| a.size <=> b.size }
end
...
def handle_start_with_capital
    count = 0
    handles.each do |handle|
        if handle.start_with?(/\S[A-Z]/)
            #puts handle
            count += 1
        end
    end
    puts "Les handle commencant par une majuscule sont au nombre de :"
    puts count
end
